...
impl ModeInfo {
    ...
    pub fn dotclock(&self) -> Dotclock {
        unsafe { self.base.dotclock }
    }
    pub fn hdisplay(&self) -> u16 {
        unsafe { self.base.hdisplay }
    }
    pub fn hsyncstart(&self) -> u16 {
        unsafe { self.base.hsyncstart }
    }
    pub fn hsyncend(&self) -> u16 {
        unsafe { self.base.hsyncend }
    }
    ...
    pub fn vdisplay(&self) -> u16 {
        unsafe { self.base.vdisplay }
    }
    pub fn vsyncstart(&self) -> u16 {
        unsafe { self.base.vsyncstart }
    }
    pub fn vsyncend(&self) -> u16 {
        unsafe { self.base.vsyncend }
    }
...
impl GetModeLineReply {
    pub fn dotclock(&self) -> Dotclock {
        unsafe { (*self.ptr).dotclock }
    }
    pub fn hdisplay(&self) -> u16 {
        unsafe { (*self.ptr).hdisplay }
    }
    pub fn hsyncstart(&self) -> u16 {
        unsafe { (*self.ptr).hsyncstart }
    }
    pub fn hsyncend(&self) -> u16 {
        unsafe { (*self.ptr).hsyncend }
    }
    pub fn htotal(&self) -> u16 {
        unsafe { (*self.ptr).htotal }
    }
    pub fn hskew(&self) -> u16 {
        unsafe { (*self.ptr).hskew }
    }
    pub fn vdisplay(&self) -> u16 {
        unsafe { (*self.ptr).vdisplay }
    }
    pub fn vsyncstart(&self) -> u16 {
        unsafe { (*self.ptr).vsyncstart }
    }
    pub fn vsyncend(&self) -> u16 {
        unsafe { (*self.ptr).vsyncend }
    }
    pub fn vtotal(&self) -> u16 {
        unsafe { (*self.ptr).vtotal }
    }
    pub fn flags(&self) -> u32 {
        unsafe { (*self.ptr).flags }
    }
    pub fn privsize(&self) -> u32 {
        unsafe { (*self.ptr).privsize }
    }
...
pub fn mod_mode_line<'a>(
    ...
    unsafe {
        ...
        let cookie = xcb_xf86vidmode_mod_mode_line(
            c.get_raw_conn(),
            screen as u32,
            hdisplay as u16,
            hsyncstart as u16,
...
pub fn mod_mode_line_checked<'a>(
    c: &'a base::Connection,
    screen: u32,
    hdisplay: u16,
    hsyncstart: u16,
    ...
    unsafe {
        let private_len = private.len();
        let private_ptr = private.as_ptr();
        let cookie = xcb_xf86vidmode_mod_mode_line_checked(
            c.get_raw_conn(),
            screen as u32,
            hdisplay as u16,
            hsyncstart as u16,
...
impl GetMonitorReply {
    pub fn vendor_length(&self) -> u8 {
        unsafe { (*self.ptr).vendor_length }
    }
    pub fn model_length(&self) -> u8 {
        unsafe { (*self.ptr).model_length }
    }
    pub fn num_hsync(&self) -> u8 {
        unsafe { (*self.ptr).num_hsync }
    }
    pub fn num_vsync(&self) -> u8 {
        unsafe { (*self.ptr).num_vsync }
    }
...
pub fn lock_mode_switch_checked<'a>(
    c: &'a base::Connection,
    screen: u16,
    lock: u16,
) -> base::VoidCookie<'a> {
...
impl GetAllModeLinesReply {
    pub fn modecount(&self) -> u32 {
        unsafe { (*self.ptr).modecount }
    }
    pub fn modeinfo(&self) -> ModeInfoIterator {
        unsafe { xcb_xf86vidmode_get_all_mode_lines_modeinfo_iterator(self.ptr) }
    }
}
...
pub fn get_all_mode_lines_unchecked<'a>(
    c: &'a base::Connection,
    screen: u16,
) -> GetAllModeLinesCookie<'a> {
    unsafe {
...
pub fn add_mode_line<'a>(
    ...
    unsafe {
        let private_len = private.len();
        let private_ptr = private.as_ptr();
        let cookie = xcb_xf86vidmode_add_mode_line(
            c.get_raw_conn(),
            screen as u32,
            dotclock as xcb_xf86vidmode_dotclock_t,
            hdisplay as u16,
            hsyncstart as u16,
            hsyncend as u16,
            htotal as u16,
            ...
            flags as u32,
            private_len as u32,
            after_dotclock as xcb_xf86vidmode_dotclock_t,
            after_hdisplay as u16,
            after_hsyncstart as u16,
            after_hsyncend as u16,
            after_htotal as u16,
            after_hskew as u16,
            after_vdisplay as u16,
            after_vsyncstart as u16,
            after_vsyncend as u16,
            after_vtotal as u16,
            after_flags as u32,
            private_ptr as *const u8,
        );
...
pub fn add_mode_line_checked<'a>(
    c: &'a base::Connection,
    screen: u32,
    dotclock: Dotclock,
    hdisplay: u16,
    ...
    unsafe {
        let private_len = private.len();
        let private_ptr = private.as_ptr();
        let cookie = xcb_xf86vidmode_add_mode_line_checked(
            c.get_raw_conn(),
            screen as u32,
            dotclock as xcb_xf86vidmode_dotclock_t,
            hdisplay as u16,
...
pub fn delete_mode_line<'a>(
    ...
    unsafe {
        let private_len = private.len();
        let private_ptr = private.as_ptr();
        let cookie = xcb_xf86vidmode_delete_mode_line(
            c.get_raw_conn(),
            screen as u32,
            dotclock as xcb_xf86vidmode_dotclock_t,
            hdisplay as u16,
...
pub fn delete_mode_line_checked<'a>(
    c: &'a base::Connection,
    screen: u32,
    dotclock: Dotclock,
    hdisplay: u16,
    ...
    unsafe {
        let private_len = private.len();
        let private_ptr = private.as_ptr();
        let cookie = xcb_xf86vidmode_delete_mode_line_checked(
            c.get_raw_conn(),
            screen as u32,
            dotclock as xcb_xf86vidmode_dotclock_t,
            hdisplay as u16,
...
pub fn validate_mode_line<'a>(
    ...
    unsafe {
        let private_len = private.len();
        let private_ptr = private.as_ptr();
        let cookie = xcb_xf86vidmode_validate_mode_line(
            c.get_raw_conn(),
            screen as u32,
            dotclock as xcb_xf86vidmode_dotclock_t,
            hdisplay as u16,
...
pub fn validate_mode_line_unchecked<'a>(
    c: &'a base::Connection,
    screen: u32,
    dotclock: Dotclock,
    hdisplay: u16,
    ...
    unsafe {
        let private_len = private.len();
        let private_ptr = private.as_ptr();
        let cookie = xcb_xf86vidmode_validate_mode_line_unchecked(
            c.get_raw_conn(),
            screen as u32,
            dotclock as xcb_xf86vidmode_dotclock_t,
            hdisplay as u16,
...
pub fn switch_to_mode<'a>(
    ...
    unsafe {
        let private_len = private.len();
        let private_ptr = private.as_ptr();
        let cookie = xcb_xf86vidmode_switch_to_mode(
            c.get_raw_conn(),
            screen as u32,
            dotclock as xcb_xf86vidmode_dotclock_t,
            hdisplay as u16,
...
pub fn switch_to_mode_checked<'a>(
    c: &'a base::Connection,
    screen: u32,
    dotclock: Dotclock,
    hdisplay: u16,
    ...
    unsafe {
        let private_len = private.len();
        let private_ptr = private.as_ptr();
        let cookie = xcb_xf86vidmode_switch_to_mode_checked(
            c.get_raw_conn(),
            screen as u32,
            dotclock as xcb_xf86vidmode_dotclock_t,
            hdisplay as u16,
...
pub fn set_view_port_checked<'a>(
    c: &'a base::Connection,
    screen: u16,
    x: u32,
    y: u32,
    ...
    unsafe {
        let cookie = xcb_xf86vidmode_set_view_port_checked(
            c.get_raw_conn(),
            screen as u16,
            x as u32,
            y as u32,
...
impl GetDotClocksReply {
    ...
    pub fn clocks(&self) -> u32 {
        unsafe { (*self.ptr).clocks }
    }
    pub fn maxclocks(&self) -> u32 {
        unsafe { (*self.ptr).maxclocks }
    }
...
pub fn get_dot_clocks_unchecked<'a>(
    c: &'a base::Connection,
    screen: u16,
) -> GetDotClocksCookie<'a> {
    unsafe {
...
pub fn set_client_version_checked<'a>(
    c: &'a base::Connection,
    major: u16,
    minor: u16,
) -> base::VoidCookie<'a> {
    unsafe {
        let cookie = xcb_xf86vidmode_set_client_version_checked(
            c.get_raw_conn(),
            major as u16,
            minor as u16,
        );
...
pub fn set_gamma<'a>(
    ...
    unsafe {
        let cookie = xcb_xf86vidmode_set_gamma(
            c.get_raw_conn(),
            screen as u16,
            red as u32,
            green as u32,
...
pub fn set_gamma_checked<'a>(
    ...
    unsafe {
        let cookie = xcb_xf86vidmode_set_gamma_checked(
            c.get_raw_conn(),
            screen as u16,
            red as u32,
            green as u32,
...
pub fn get_gamma_ramp_unchecked<'a>(
    c: &'a base::Connection,
    screen: u16,
    size: u16,
) -> GetGammaRampCookie<'a> {
...
pub fn set_gamma_ramp<'a>(
    ...
    unsafe {
        ...
        let cookie = xcb_xf86vidmode_set_gamma_ramp(
            c.get_raw_conn(),
            screen as u16,
            red_len as u16,
...
pub fn set_gamma_ramp_checked<'a>(
    c: &'a base::Connection,
    screen: u16,
    red: &[u16],
    green: &[u16],
    ...
    unsafe {
        ...
        let green_ptr = green.as_ptr();
        let blue_ptr = blue.as_ptr();
        let cookie = xcb_xf86vidmode_set_gamma_ramp_checked(
            c.get_raw_conn(),
            screen as u16,
            red_len as u16,
            red_ptr as *const u16,
...
pub fn get_gamma_ramp_size_unchecked<'a>(
    c: &'a base::Connection,
    screen: u16,
) -> GetGammaRampSizeCookie<'a> {
    unsafe {
...
pub fn get_permissions_unchecked<'a>(
    c: &'a base::Connection,
    screen: u16,
) -> GetPermissionsCookie<'a> {
    unsafe {
