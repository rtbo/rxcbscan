...
impl<'a> QueryVersionCookie<'a> {
    pub fn get_reply(self) -> Result<QueryVersionReply, base::ReplyError> {
        ...
        let reply = unsafe {
            QueryVersionReply {
                ptr: xcb_composite_query_version_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
    ...
    pub fn get_reply(self) -> Result<GetOverlayWindowReply, base::ReplyError> {
        ...
        let reply = unsafe {
            GetOverlayWindowReply {
                ptr: xcb_composite_get_overlay_window_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
        let checked = self.checked;
        std::mem::forget(self); // won't call discard on cookie
