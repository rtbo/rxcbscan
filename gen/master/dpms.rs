...
pub fn get_version<'a>(
    ...
    unsafe {
        let cookie = xcb_dpms_get_version(
            c.get_raw_conn(),
...
pub fn get_version_unchecked<'a>(
    ...
    unsafe {
        let cookie = xcb_dpms_get_version_unchecked(
            c.get_raw_conn(),
            client_major_version as u16,
            client_minor_version as u16,
        );
...
impl GetTimeoutsReply {
    pub fn standby_timeout(&self) -> u16 {
        unsafe { (*self.ptr).standby_timeout }
    }
    pub fn suspend_timeout(&self) -> u16 {
        unsafe { (*self.ptr).suspend_timeout }
    }
    pub fn off_timeout(&self) -> u16 {
        unsafe { (*self.ptr).off_timeout }
    }
}
...
pub fn set_timeouts<'a>(
    ...
    unsafe {
        let cookie = xcb_dpms_set_timeouts(
            c.get_raw_conn(),
            standby_timeout as u16,
            suspend_timeout as u16,
            off_timeout as u16,
        );
        base::VoidCookie {
            cookie: cookie,
...
pub fn set_timeouts_checked<'a>(
    ...
    unsafe {
        let cookie = xcb_dpms_set_timeouts_checked(
            c.get_raw_conn(),
            standby_timeout as u16,
            suspend_timeout as u16,
            off_timeout as u16,
...
impl InfoReply {
    pub fn power_level(&self) -> u16 {
        unsafe { (*self.ptr).power_level }
    }
    pub fn state(&self) -> bool {
        unsafe { (*self.ptr).state != 0 }
    }
}
