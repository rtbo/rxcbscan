...
impl<'a> QueryVersionCookie<'a> {
    pub fn get_reply(self) -> Result<QueryVersionReply, base::ReplyError> {
        ...
        let reply = unsafe {
            QueryVersionReply {
                ptr: xcb_present_query_version_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
    ...
    pub fn get_reply(self) -> Result<QueryCapabilitiesReply, base::ReplyError> {
        ...
        let reply = unsafe {
            QueryCapabilitiesReply {
                ptr: xcb_present_query_capabilities_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
        let checked = self.checked;
        std::mem::forget(self); // won't call discard on cookie
