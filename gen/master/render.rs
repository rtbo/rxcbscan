    ...
    pub fn get_reply(self) -> Result<QueryPictFormatsReply, base::ReplyError> {
        ...
        let reply = unsafe {
            QueryPictFormatsReply {
                ptr: xcb_render_query_pict_formats_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
        let checked = self.checked;
        std::mem::forget(self); // won't call discard on cookie
    ...
    pub fn get_reply(self) -> Result<QueryPictIndexValuesReply, base::ReplyError> {
        ...
        let reply = unsafe {
            QueryPictIndexValuesReply {
                ptr: xcb_render_query_pict_index_values_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
        let checked = self.checked;
        std::mem::forget(self); // won't call discard on cookie
