...
impl<'a> QueryVersionCookie<'a> {
    pub fn get_reply(self) -> Result<QueryVersionReply, base::ReplyError> {
        ...
        let reply = unsafe {
            QueryVersionReply {
                ptr: xcb_screensaver_query_version_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
...
impl<'a> QueryInfoCookie<'a> {
    pub fn get_reply(self) -> Result<QueryInfoReply, base::ReplyError> {
        ...
        let reply = unsafe {
            QueryInfoReply {
                ptr: xcb_screensaver_query_info_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
        let checked = self.checked;
        std::mem::forget(self); // won't call discard on cookie
