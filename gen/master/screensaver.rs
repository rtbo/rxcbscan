...
pub fn query_version<'a>(
    ...
    unsafe {
        let cookie = xcb_screensaver_query_version(
            c.get_raw_conn(),
            client_major_version as u8,
            client_minor_version as u8,
        );
        QueryVersionCookie {
            cookie: cookie,
            conn: c,
...
pub fn query_version_unchecked<'a>(
    ...
    unsafe {
        let cookie = xcb_screensaver_query_version_unchecked(
            c.get_raw_conn(),
            client_major_version as u8,
            client_minor_version as u8,
        );
...
impl QueryInfoReply {
    ...
    pub fn saver_window(&self) -> xproto::Window {
        unsafe { (*self.ptr).saver_window }
    }
    pub fn ms_until_server(&self) -> u32 {
        unsafe { (*self.ptr).ms_until_server }
    }
    pub fn ms_since_user_input(&self) -> u32 {
        unsafe { (*self.ptr).ms_since_user_input }
    }
...
pub fn select_input<'a>(
    ...
    unsafe {
        let cookie = xcb_screensaver_select_input(
            c.get_raw_conn(),
...
pub fn select_input_checked<'a>(
    ...
    unsafe {
        let cookie = xcb_screensaver_select_input_checked(
            c.get_raw_conn(),
            drawable as xcb_drawable_t,
            event_mask as u32,
        );
...
pub fn set_attributes<'a>(
    ...
    unsafe {
        ...
        let cookie = xcb_screensaver_set_attributes(
            c.get_raw_conn(),
            drawable as xcb_drawable_t,
            x as i16,
            y as i16,
...
pub fn set_attributes_checked<'a>(
    c: &'a base::Connection,
    drawable: xproto::Drawable,
    x: i16,
    y: i16,
    ...
    unsafe {
        ...
        let (value_list_mask, value_list_vec) = base::pack_bitfield(&mut value_list_copy);
        let value_list_ptr = value_list_vec.as_ptr();
        let cookie = xcb_screensaver_set_attributes_checked(
            c.get_raw_conn(),
            drawable as xcb_drawable_t,
            x as i16,
            y as i16,
...
pub fn unset_attributes_checked<'a>(
    c: &'a base::Connection,
    drawable: xproto::Drawable,
) -> base::VoidCookie<'a> {
    unsafe {
