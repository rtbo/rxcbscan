...
impl<'a> Systemcounter<'a> {
    pub fn counter(&self) -> Counter {
        unsafe { (*self.ptr).counter }
    }
    pub fn resolution(&self) -> Int64 {
        unsafe { std::mem::transmute((*self.ptr).resolution) }
    }
...
impl Trigger {
    ...
    pub fn counter(&self) -> Counter {
        unsafe { self.base.counter }
    }
    pub fn wait_type(&self) -> u32 {
        unsafe { self.base.wait_type }
    }
    pub fn wait_value(&self) -> Int64 {
        unsafe { std::mem::transmute(self.base.wait_value) }
    }
    pub fn test_type(&self) -> u32 {
        unsafe { self.base.test_type }
    }
}
...
impl Waitcondition {
    ...
    pub fn trigger(&self) -> Trigger {
        unsafe { std::mem::transmute(self.base.trigger) }
    }
    pub fn event_threshold(&self) -> Int64 {
        unsafe { std::mem::transmute(self.base.event_threshold) }
    }
}
...
pub fn initialize<'a>(
    ...
    unsafe {
        let cookie = xcb_sync_initialize(
            c.get_raw_conn(),
            desired_major_version as u8,
            desired_minor_version as u8,
        );
        InitializeCookie {
            cookie: cookie,
            conn: c,
...
pub fn initialize_unchecked<'a>(
    c: &'a base::Connection,
    desired_major_version: u8,
    desired_minor_version: u8,
) -> InitializeCookie<'a> {
    unsafe {
        let cookie = xcb_sync_initialize_unchecked(
            c.get_raw_conn(),
            desired_major_version as u8,
            desired_minor_version as u8,
        );
...
impl ListSystemCountersReply {
    pub fn counters_len(&self) -> u32 {
        unsafe { (*self.ptr).counters_len }
    }
    pub fn counters(&self) -> SystemcounterIterator {
        unsafe { xcb_sync_list_system_counters_counters_iterator(self.ptr) }
    }
}
...
pub fn create_counter<'a>(
    c: &'a base::Connection,
    id: Counter,
    initial_value: Int64,
) -> base::VoidCookie<'a> {
    unsafe {
        let cookie = xcb_sync_create_counter(
            c.get_raw_conn(),
...
pub fn create_counter_checked<'a>(
    c: &'a base::Connection,
    id: Counter,
    initial_value: Int64,
) -> base::VoidCookie<'a> {
    unsafe {
        let cookie = xcb_sync_create_counter_checked(
            c.get_raw_conn(),
            id as xcb_sync_counter_t,
            initial_value.base,
        );
...
pub fn destroy_counter_checked<'a>(
    c: &'a base::Connection,
    counter: Counter,
) -> base::VoidCookie<'a> {
    unsafe {
...
impl QueryCounterReply {
    pub fn counter_value(&self) -> Int64 {
        unsafe { std::mem::transmute((*self.ptr).counter_value) }
    }
}
...
pub fn query_counter_unchecked<'a>(
    c: &'a base::Connection,
    counter: Counter,
...
pub fn await_<'a>(c: &'a base::Connection, wait_list: &[Waitcondition]) -> base::VoidCookie<'a> {
    unsafe {
        ...
        let cookie = xcb_sync_await(
            c.get_raw_conn(),
            wait_list_len as u32,
            wait_list_ptr as *const xcb_sync_waitcondition_t,
        );
        base::VoidCookie {
            cookie: cookie,
            conn: c,
...
pub fn await_checked<'a>(
    ...
    unsafe {
        let wait_list_len = wait_list.len();
        let wait_list_ptr = wait_list.as_ptr();
        let cookie = xcb_sync_await_checked(
            c.get_raw_conn(),
            wait_list_len as u32,
            wait_list_ptr as *const xcb_sync_waitcondition_t,
        );
...
pub fn change_counter_checked<'a>(
    c: &'a base::Connection,
    counter: Counter,
    amount: Int64,
) -> base::VoidCookie<'a> {
    unsafe {
        let cookie = xcb_sync_change_counter_checked(
            c.get_raw_conn(),
...
pub fn set_counter_checked<'a>(
    c: &'a base::Connection,
    counter: Counter,
    value: Int64,
) -> base::VoidCookie<'a> {
    unsafe {
        let cookie = xcb_sync_set_counter_checked(
            c.get_raw_conn(),
            counter as xcb_sync_counter_t,
            value.base,
        );
...
pub fn create_alarm<'a>(
    ...
    unsafe {
        ...
        let cookie = xcb_sync_create_alarm(
            c.get_raw_conn(),
...
pub fn create_alarm_checked<'a>(
    ...
    unsafe {
        ...
        let cookie = xcb_sync_create_alarm_checked(
            c.get_raw_conn(),
            id as xcb_sync_alarm_t,
            value_mask as u32,
            value_list_ptr,
...
pub fn change_alarm<'a>(
    ...
    unsafe {
        ...
        let cookie = xcb_sync_change_alarm(
            c.get_raw_conn(),
            id as xcb_sync_alarm_t,
            value_mask as u32,
            value_list_ptr,
...
pub fn change_alarm_checked<'a>(
    ...
    unsafe {
        ...
        let cookie = xcb_sync_change_alarm_checked(
            c.get_raw_conn(),
            id as xcb_sync_alarm_t,
            value_mask as u32,
            value_list_ptr,
...
impl QueryAlarmReply {
    pub fn trigger(&self) -> Trigger {
        unsafe { std::mem::transmute((*self.ptr).trigger) }
    }
    pub fn delta(&self) -> Int64 {
        unsafe { std::mem::transmute((*self.ptr).delta) }
    }
    pub fn events(&self) -> bool {
        unsafe { (*self.ptr).events != 0 }
    }
...
pub fn create_fence<'a>(
    ...
    unsafe {
        let cookie = xcb_sync_create_fence(
            c.get_raw_conn(),
...
pub fn create_fence_checked<'a>(
    ...
    unsafe {
        let cookie = xcb_sync_create_fence_checked(
            c.get_raw_conn(),
            drawable as xcb_drawable_t,
            fence as xcb_sync_fence_t,
            initially_triggered as u8,
...
pub fn await_fence<'a>(c: &'a base::Connection, fence_list: &[Fence]) -> base::VoidCookie<'a> {
    unsafe {
        ...
        let cookie = xcb_sync_await_fence(
            c.get_raw_conn(),
            fence_list_len as u32,
            fence_list_ptr as *const xcb_sync_fence_t,
        );
        base::VoidCookie {
            cookie: cookie,
            conn: c,
...
pub fn await_fence_checked<'a>(
    ...
    unsafe {
        let fence_list_len = fence_list.len();
        let fence_list_ptr = fence_list.as_ptr();
        let cookie = xcb_sync_await_fence_checked(
            c.get_raw_conn(),
            fence_list_len as u32,
            fence_list_ptr as *const xcb_sync_fence_t,
        );
...
impl CounterNotifyEvent {
    ...
    pub fn wait_value(&self) -> Int64 {
        unsafe { std::mem::transmute((*self.ptr).wait_value) }
    }
    pub fn counter_value(&self) -> Int64 {
        unsafe { std::mem::transmute((*self.ptr).counter_value) }
    }
...
impl AlarmNotifyEvent {
    ...
    pub fn alarm(&self) -> Alarm {
        unsafe { (*self.ptr).alarm }
    }
    pub fn counter_value(&self) -> Int64 {
        unsafe { std::mem::transmute((*self.ptr).counter_value) }
    }
    pub fn alarm_value(&self) -> Int64 {
        unsafe { std::mem::transmute((*self.ptr).alarm_value) }
    }
    pub fn timestamp(&self) -> xproto::Timestamp {
        unsafe { (*self.ptr).timestamp }
    }
