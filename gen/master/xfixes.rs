...
impl<'a> GetCursorImageCookie<'a> {
    pub fn get_reply(self) -> Result<GetCursorImageReply, base::ReplyError> {
        ...
        let reply = unsafe {
            GetCursorImageReply {
                ptr: xcb_xfixes_get_cursor_image_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
        let checked = self.checked;
        std::mem::forget(self); // won't call discard on cookie
...
impl<'a> GetCursorNameCookie<'a> {
    pub fn get_reply(self) -> Result<GetCursorNameReply, base::ReplyError> {
        ...
        let reply = unsafe {
            GetCursorNameReply {
                ptr: xcb_xfixes_get_cursor_name_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
        let checked = self.checked;
        std::mem::forget(self); // won't call discard on cookie
    ...
    pub fn get_reply(self) -> Result<GetCursorImageAndNameReply, base::ReplyError> {
        ...
        let reply = unsafe {
            GetCursorImageAndNameReply {
                ptr: xcb_xfixes_get_cursor_image_and_name_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
        let checked = self.checked;
        std::mem::forget(self); // won't call discard on cookie
