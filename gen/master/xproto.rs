    ...
    pub fn get_reply(self) -> Result<GetWindowAttributesReply, base::ReplyError> {
        ...
        let reply = unsafe {
            GetWindowAttributesReply {
                ptr: xcb_get_window_attributes_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
    ...
    pub fn get_reply(self) -> Result<TranslateCoordinatesReply, base::ReplyError> {
        ...
        let reply = unsafe {
            TranslateCoordinatesReply {
                ptr: xcb_translate_coordinates_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
        let checked = self.checked;
        std::mem::forget(self); // won't call discard on cookie
    ...
    pub fn get_reply(self) -> Result<ListInstalledColormapsReply, base::ReplyError> {
        ...
        let reply = unsafe {
            ListInstalledColormapsReply {
                ptr: xcb_list_installed_colormaps_reply (self.conn.get_raw_conn(), self.cookie, err_ptr)
            }
        };
        let checked = self.checked;
        std::mem::forget(self); // won't call discard on cookie
